...
namespace Voxel {
    BufferAllocator::BufferAllocator() {
        ...
        for (size_t i {0}; i < MAX_OBJECTS; ++i) {
            ...
            glBufferStorage(GL_ARRAY_BUFFER, sizeof(uint32_t) * 20000, nullptr, flags);
            vertex_buffer_objects[i] = (void*)glMapBufferRange(GL_ARRAY_BUFFER, 0, sizeof(uint32_t) * 20000, flags);

            glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, ebo_ids[i]);
            glBufferStorage(GL_ELEMENT_ARRAY_BUFFER, sizeof(unsigned int) * 20000, nullptr, flags);
            element_buffer_objects[i] = (void*)glMapBufferRange(GL_ELEMENT_ARRAY_BUFFER, 0, sizeof(unsigned int) * 20000, flags);
