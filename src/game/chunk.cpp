...
namespace Voxel::Game {
    ...
    std::shared_ptr<Chunk> Chunk::create(int* height_map, BlockType* block_types, std::vector<glm::ivec2>& tree_positions, Noise& noise, glm::ivec3 position) {
    ...
    void Chunk::generate_trees(int* height_map, std::vector<glm::ivec2>& tree_positions) {
        const int stemm_height = 2 + rand() % 3;
        ...
        for (auto& tree_position : tree_positions) {
            ...
            for (int y = ground_y_chunk_space + stemm_height; y < ground_y_chunk_space + stemm_height + leafs_height; y++) {
                ...
                for (uint16_t x = 0; x < SIZE; x++)
                {
                    ...
                    if (block == BlockType::Air) {
                        if (world_space_position_y == 0)
                            ...
                            else if (noise.fetch_cave(position.x + x, position.y + y, position.z + z) < .7f)
    ...
    void Chunk::render() {
        ...
        if (!instance.get() && mesh->triangles > 0) {
            ...
            ssbo->bind();
            ssbo->data(0, (unsigned int*)block_types_ptr, sizeof(unsigned int) * 16 * 16 * 16);
            ssbo->unbind();
        }
